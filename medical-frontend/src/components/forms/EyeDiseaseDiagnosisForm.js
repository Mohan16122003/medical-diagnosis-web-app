...
const EyeDiseaseDiagnosisForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post("http://127.0.0.1:5000/predict/eye_disease", formData, {
                headers: { "Content-Type": "multipart/form-data" }
            });

            if (response.data.class && response.data.confidence !== undefined) {
                const DiseaseType = response.data.class;
    ...
    return (
        <div className="form-container">
            <h2 className="form-title">Eye Disease Detection</h2>
...
export default EyeDiseaseDiagnosisForm;
